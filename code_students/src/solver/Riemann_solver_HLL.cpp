...
void HLL_solver::get_num_flux(fluid_cell &fluid_left_cell, fluid_cell &fluid_right_cell, const std::vector<double> &phys_flux_left_cell,
	...
	if (v_char_slowest > 0.0) {
		...
		for (std::size_t i_field = 0; i_field < num_fields; i_field++) {
			double top1 = v_char_fastest*phys_flux_left_cell[i_field]-v_char_slowest*phys_flux_right_cell[i_field];
			double top2 = v_char_fastest*v_char_slowest*(fluid_right_cell.fluid_data[i_field]-fluid_left_cell.fluid_data[i_field]);
			double bottom = v_char_fastest-v_char_slowest+epsilon;
			num_flux[i_field] = (top1+top2)/bottom;
		}
	}
}
